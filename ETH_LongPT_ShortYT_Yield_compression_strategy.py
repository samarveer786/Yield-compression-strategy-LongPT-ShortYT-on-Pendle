#!/usr/bin/env python
# coding: utf-8

# In[ ]:


pip install requests pandas pyarrow tqdm


# In[1]:


import requests
import pandas as pd
import time
from datetime import datetime, timedelta, timezone

BASE_URL = "https://api-v2.pendle.finance/core"
CHAIN_ID = 1  # Ethereum only
LOOKBACK_MONTHS = 9
MIN_DAYS_TO_EXPIRY = 21


# In[2]:


def safe_get(url, params):
    r = requests.get(url, params=params, timeout=30)
    if r.status_code == 429:
        time.sleep(8)
        r = requests.get(url, params=params, timeout=30)
    r.raise_for_status()
    return r


def get_all_markets():
    url = f"{BASE_URL}/v1/markets/all"
    params = {"chainId": CHAIN_ID}
    r = safe_get(url, params)
    return pd.DataFrame(r.json()["markets"])


def get_market_history(market_address):
    start_ts = datetime.utcnow() - timedelta(days=30 * LOOKBACK_MONTHS)

    url = f"{BASE_URL}/v2/{CHAIN_ID}/markets/{market_address}/historical-data"
    params = {
        "time_frame": "day",
        "fields": "timestamp,ptPrice,ytPrice,impliedApy,underlyingApy,tradingVolume,tvl",
        "timestamp_start": start_ts.isoformat()
    }

    r = safe_get(url, params)
    data = r.json()["results"]

    if not data:
        return None

    df = pd.DataFrame(data)
    df["timestamp"] = pd.to_datetime(df["timestamp"])
    df.set_index("timestamp", inplace=True)
    return df


# In[3]:


# ==========================
# ASSET: wstETH
# ==========================

markets = get_all_markets()
now = datetime.now(timezone.utc)

df_wstETH = []

for _, row in markets.iterrows():
    name = str(row.get("name", "")).lower()

    if "wsteth" not in name:
        continue
    if "bera" in name:
        continue

    expiry = row.get("expiry")
    if expiry is None:
        continue

    expiry_dt = pd.to_datetime(expiry, utc=True)
    if (expiry_dt - now).days < MIN_DAYS_TO_EXPIRY:
        continue

    df = get_market_history(row["address"])
    if df is None:
        continue

    df["asset"] = "wstETH"
    df["market"] = row["name"]
    df["expiry"] = expiry_dt
    df["market_address"] = row["address"]

    df_wstETH.append(df)

df_wstETH = pd.concat(df_wstETH).sort_index()


# In[4]:


# ==========================
# ASSET: swETH
# ==========================

markets = get_all_markets()
now = datetime.now(timezone.utc)

df_swETH = []

for _, row in markets.iterrows():
    name = str(row.get("name", "")).lower()

    if "sweth" not in name:
        continue
    if "bera" in name:
        continue

    expiry = row.get("expiry")
    if expiry is None:
        continue

    expiry_dt = pd.to_datetime(expiry, utc=True)
    if (expiry_dt - now).days < MIN_DAYS_TO_EXPIRY:
        continue

    df = get_market_history(row["address"])
    if df is None:
        continue

    df["asset"] = "swETH"
    df["market"] = row["name"]
    df["expiry"] = expiry_dt
    df["market_address"] = row["address"]

    df_swETH.append(df)

df_swETH = pd.concat(df_swETH).sort_index()


# In[5]:


# ==========================
# ASSET: rsETH
# ==========================

markets = get_all_markets()
now = datetime.now(timezone.utc)

df_rsETH = []

for _, row in markets.iterrows():
    name = str(row.get("name", "")).lower()

    if "rseth" not in name:
        continue
    if "bera" in name:
        continue

    expiry = row.get("expiry")
    if expiry is None:
        continue

    expiry_dt = pd.to_datetime(expiry, utc=True)
    if (expiry_dt - now).days < MIN_DAYS_TO_EXPIRY:
        continue

    df = get_market_history(row["address"])
    if df is None:
        continue

    df["asset"] = "rsETH"
    df["market"] = row["name"]
    df["expiry"] = expiry_dt
    df["market_address"] = row["address"]

    df_rsETH.append(df)

df_rsETH = pd.concat(df_rsETH).sort_index()


# In[6]:


# ==========================
# ASSET: weETH
# ==========================

markets = get_all_markets()
now = datetime.now(timezone.utc)

df_weETH = []

for _, row in markets.iterrows():
    name = str(row.get("name", "")).lower()

    if "weeth" not in name:
        continue
    if "bera" in name:
        continue

    expiry = row.get("expiry")
    if expiry is None:
        continue

    expiry_dt = pd.to_datetime(expiry, utc=True)
    if (expiry_dt - now).days < MIN_DAYS_TO_EXPIRY:
        continue

    df = get_market_history(row["address"])
    if df is None:
        continue

    df["asset"] = "weETH"
    df["market"] = row["name"]
    df["expiry"] = expiry_dt
    df["market_address"] = row["address"]

    df_weETH.append(df)

df_weETH = pd.concat(df_weETH).sort_index()


# In[7]:


# ==========================
# ASSET: pufETH
# ==========================

markets = get_all_markets()
now = datetime.now(timezone.utc)

df_pufETH = []

for _, row in markets.iterrows():
    name = str(row.get("name", "")).lower()

    if "pufeth" not in name:
        continue
    if "bera" in name:
        continue

    expiry = row.get("expiry")
    if expiry is None:
        continue

    expiry_dt = pd.to_datetime(expiry, utc=True)
    if (expiry_dt - now).days < MIN_DAYS_TO_EXPIRY:
        continue

    df = get_market_history(row["address"])
    if df is None:
        continue

    df["asset"] = "pufETH"
    df["market"] = row["name"]
    df["expiry"] = expiry_dt
    df["market_address"] = row["address"]

    df_pufETH.append(df)

df_pufETH = pd.concat(df_pufETH).sort_index()


# In[8]:


# ==========================
# ASSET: uniETH
# ==========================

markets = get_all_markets()
now = datetime.now(timezone.utc)

df_uniETH = []

for _, row in markets.iterrows():
    name = str(row.get("name", "")).lower()

    if "unieth" not in name:
        continue
    if "bera" in name:
        continue

    expiry = row.get("expiry")
    if expiry is None:
        continue

    expiry_dt = pd.to_datetime(expiry, utc=True)
    if (expiry_dt - now).days < MIN_DAYS_TO_EXPIRY:
        continue

    df = get_market_history(row["address"])
    if df is None:
        continue

    df["asset"] = "uniETH"
    df["market"] = row["name"]
    df["expiry"] = expiry_dt
    df["market_address"] = row["address"]

    df_uniETH.append(df)

df_uniETH = pd.concat(df_uniETH).sort_index()


# In[9]:


# ==========================
# ASSET: hgETH
# ==========================

markets = get_all_markets()
now = datetime.now(timezone.utc)

df_hgETH = []

for _, row in markets.iterrows():
    name = str(row.get("name", "")).lower()

    if "hgeth" not in name:
        continue
    if "bera" in name:
        continue

    expiry = row.get("expiry")
    if expiry is None:
        continue

    expiry_dt = pd.to_datetime(expiry, utc=True)
    if (expiry_dt - now).days < MIN_DAYS_TO_EXPIRY:
        continue

    df = get_market_history(row["address"])
    if df is None:
        continue

    df["asset"] = "hgETH"
    df["market"] = row["name"]
    df["expiry"] = expiry_dt
    df["market_address"] = row["address"]

    df_hgETH.append(df)

df_hgETH = pd.concat(df_hgETH).sort_index()


# In[10]:


df = df.drop(columns=["market"], errors="ignore")


# In[11]:


final_df = pd.concat([
    df_wstETH,
    df_swETH,
    df_rsETH,
    df_weETH,
    df_pufETH,
    df_uniETH,
    df_hgETH
])


# In[12]:


### Adding Yield_gap and maturity columns
df = final_df.copy()

df["days_to_expiry"] = (df["expiry"] - df.index).dt.days
df["yield_gap"] = df["impliedApy"] - df["underlyingApy"]


df.drop(columns=["market"], errors="ignore")


# In[13]:


def norm_window(days_to_expiry):
    return int(min(30, max(10, days_to_expiry / 4)))
df["expiry"] = pd.to_datetime(df["expiry"])
df["days_to_expiry"] = (df["expiry"] - df.index).dt.days
market_windows = (
    df.groupby("market_address")["days_to_expiry"]
      .first()
      .apply(norm_window)
)
df["yield_gap_std"] = (
    df.groupby("market_address")["yield_gap"]
      .transform(
          lambda x: x.rolling(
              window=market_windows.loc[x.name],
              min_periods=10
          ).std()
      )
)
df["yield_gap_norm"] = df["yield_gap"] / df["yield_gap_std"]


# In[14]:


df = df.sort_values(["market_address", df.index.name])
EMA_SPAN = 10  # 14-day EMA

df["ema_14"] = (
    df.groupby("market_address")["yield_gap_norm"]
      .transform(lambda x: x.ewm(span=EMA_SPAN, adjust=False).mean())
)
df["ema_14_slope"] = (
    df.groupby("market_address")["ema_14"]
      .diff()
)
df["rolling_std_7d"] = (
    df.groupby("market_address")["yield_gap_norm"]
      .transform(lambda x: x.rolling(window=7, min_periods=7).std())
)

df["rolling_std_14d"] = (
    df.groupby("market_address")["yield_gap_norm"]
      .transform(lambda x: x.rolling(window=14, min_periods=14).std())
)
df.head()


# In[15]:


df.groupby("asset")["market_address"].nunique()


# In[27]:


import matplotlib.pyplot as plt

asset_name = "rsETH"

df_asset = df[df["asset"] == asset_name].copy()


# In[28]:


import matplotlib.pyplot as plt
import pandas as pd

for mkt, g in df_asset.groupby("market_address"):
    g = g.sort_index()

    # Extract maturity (assume same for all rows of this market)
    expiry = pd.to_datetime(g["expiry"].iloc[0])

    plt.figure(figsize=(14, 5))

    # --- Raw normalized yield gap ---
    plt.plot(
        g.index,
        g["yield_gap_norm"],
        label="Yield Gap (Z)",
        linewidth=2,
        alpha=0.8
    )

    # --- EMA(7) overlay ---
    plt.plot(
        g.index,
        g["ema_14"],
        label="EMA(14)",
        linewidth=2.5,
        color="orange"
    )

    # Reference lines
    plt.axhline(0, color="black", linewidth=1)
    plt.axhline(2, color="red", linestyle="--", alpha=0.6)
    plt.axhline(-2, color="red", linestyle="--", alpha=0.6)

    # Maturity vertical line
    plt.axvline(
        expiry,
        color="purple",
        linestyle=":",
        linewidth=2,
        label=f"Maturity: {expiry.date()}"
    )

    plt.title(f"Normalized Yield Gap — {asset_name} | {mkt[:6]}")
    plt.ylabel("Yield Gap (Z-score)")
    plt.xlabel("Time")
    plt.legend(fontsize=9)
    plt.grid(True)
    plt.tight_layout()
    plt.show()




# In[23]:


import numpy as np
import pandas as pd

def backtest_eth_stat_arb_single_market(df_mkt, initial_capital=1000):
    df = df_mkt.copy().sort_index()

    Z = df["yield_gap_norm"]

    entry_cond = (
        (Z >= 2) &
        (df["rolling_std_7d"] < df["rolling_std_14d"]) &
        (df["days_to_expiry"] >= 21)
    )

    trades = []
    in_trade = False

    for t in range(len(df)):
        if not in_trade and entry_cond.iloc[t]:
            entry_time = df.index[t]
            entry_idx = t
            in_trade = True

        elif in_trade:
            exit_cond = (
                ((Z.iloc[t] <= -0.5) and ((df.index[t] - entry_time).days >= 2)) or
                (df["days_to_expiry"].iloc[t] <= 3)
            )

            if exit_cond:
                exit_time = df.index[t]
                days_held = (exit_time - entry_time).days

                avg_carry = df.loc[entry_time:exit_time, "yield_gap"].mean()
                trade_return = avg_carry * (days_held / 365)

                trades.append({
                    "entry_time": entry_time,
                    "exit_time": exit_time,
                    "days_held": days_held,
                    "carry_apy": avg_carry,
                    "return_pct": trade_return
                })

                in_trade = False

    if not trades:
        return None

    trades_df = pd.DataFrame(trades)

    trades_df["capital"] = initial_capital * (1 + trades_df["return_pct"]).cumprod()

    final_equity = trades_df["capital"].iloc[-1]
    roi = (final_equity / initial_capital - 1) * 100

    running_max = trades_df["capital"].cummax()
    drawdown = (trades_df["capital"] - running_max) / running_max
    max_dd = drawdown.min() * 100

    sharpe = (
        trades_df["return_pct"].mean() /
        trades_df["return_pct"].std()
    ) * np.sqrt(len(trades_df)) if trades_df["return_pct"].std() > 0 else 0

    return {
        "num_trades": len(trades_df),
        "profitable_trades": (trades_df["return_pct"] > 0).sum(),
        "win_rate": (trades_df["return_pct"] > 0).mean(),
        "avg_days_per_trade": trades_df["days_held"].mean(),
        "final_equity": final_equity,
        "final_roi_pct": roi,
        "max_drawdown_pct": max_dd,
        "sharpe": sharpe,
        "trades": trades_df
    }


def run_backtest_for_asset(df, asset_name, initial_capital=1000):
    rows = []
    df_asset = df[df["asset"] == asset_name].copy()

    for mkt, g in df_asset.groupby("market_address"):
        if len(g) < 40:
            continue

        res = backtest_eth_stat_arb_single_market(g, initial_capital)

        if res is None:
            continue

        rows.append({
            "asset": asset_name,
            "market_address": mkt,
            "num_trades": res["num_trades"],
            "profitable_trades": res["profitable_trades"],
            "win_rate": res["win_rate"],
            "avg_days_per_trade": res["avg_days_per_trade"],
            "final_equity": res["final_equity"],
            "final_roi_pct": res["final_roi_pct"],
            "max_drawdown_pct": res["max_drawdown_pct"],
            "sharpe": res["sharpe"]
        })

    return pd.DataFrame(rows)




# In[31]:


summary = run_backtest_for_asset(
    df,
    asset_name="wstETH",
    initial_capital=1000
)

if summary.empty:
    print("❌ No valid trades for this asset")
else:
    summary = summary.sort_values("final_roi_pct", ascending=False)
    display(summary)


# In[161]:


import matplotlib.pyplot as plt

def plot_trades_with_ema(df_mkt, asset_name="Asset"):
    df = df_mkt.copy().sort_index()

    Z = df["yield_gap_norm"]
    EMA = df["ema_14"]

       # === ENTRY CONDITIONS ===
    entry_cond = (
        (Z >= 1) &
        (df["days_to_expiry"] >= 21)
    )

    exit_cond = (
        (Z <= 0.25) |
        (df["days_to_expiry"] <= 7)
    )


    entries = []
    exits = []

    in_trade = False

    for t in range(1, len(df)):
        if not in_trade and entry_cond.iloc[t]:
            entries.append(df.index[t])
            in_trade = True

        elif in_trade and exit_cond.iloc[t]:
            exits.append(df.index[t])
            in_trade = False

    # === PLOT ===
    plt.figure(figsize=(14, 6))

    plt.plot(df.index, Z, label="Yield Gap (Z)", linewidth=2)
    plt.plot(df.index, EMA, label="EMA 14", linewidth=2, linestyle="--")

    plt.scatter(
        entries,
        Z.loc[entries],
        marker="^",
        color="green",
        s=80,
        label="Entry"
    )

    plt.scatter(
        exits,
        Z.loc[exits],
        marker="v",
        color="red",
        s=80,
        label="Exit"
    )

    plt.axhline(0, color="black", linewidth=1)
    plt.axhline(2, color="red", linestyle="--", alpha=0.4)
    plt.axhline(-2, color="red", linestyle="--", alpha=0.4)

    plt.title(f"{asset_name} | Market {df['market_address'].iloc[0][:6]}")
    plt.xlabel("Time")
    plt.ylabel("Normalized Yield Gap (Z)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()


# In[123]:


for mkt, g in df_asset.groupby("market_address"):
    plot_trades_with_ema(g, asset_name="USDf")


# In[ ]:





# In[ ]:





# In[ ]:





# In[ ]:




